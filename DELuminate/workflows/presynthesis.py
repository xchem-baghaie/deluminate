...
class Presynthesis_Enumeration(FlowSpec):
    ...
    xsmiles_directory = Parameter(
        ...
        default="/mnt/d/XSMILES/BB_XSMILES/Synthesized/",
    )
    ...
    generate_delcore_and_delflex_lists = Parameter(
        "generate_delcore_and_delflex_lists",
        help="Whether to generate BB lists for DELcore and DELflex versions of an XPL scheme",
        default=True,
    )
    ...
    def start(self):
        ...
        self.input_directory = create_path(
            f"/mnt/p/Discovery Chemistry/Library Synthesis/Enumeration/Library Enumeration/{self.library_name}/presynthesis/input/"
        )
        self.output_directory = create_path(
            f"/mnt/p/Discovery Chemistry/Library Synthesis/Enumeration/Library Enumeration/{self.library_name}/presynthesis/output/"
        )
        self.input_filepath = (
            self.input_directory + self.library_name + "_presynthesis.xlsx"
        )
        logfile = self.generate_logfile_path()
        ...
        def construct_cycle_str(cycle: CycleConfig) -> str:
            ...
            Filtering for deck in this cycle: {cycle.filter_for_deck}
    ...
    def initialize_bb_list(self):
        ...
        self.next(self.query_compatible_bbs)

    @step
    def query_compatible_bbs(self):
        self.cycle.query_compatible_bbs()
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
        self.next(self.filter_deck)

    @step
    def filter_deck(self):
        if not self.generate_delcore_and_delflex_lists:
            self.cycle.filter_cycle_for_deck(self.library.deck)
        elif self.library.deck is not None:
            raise Exception(f"It was requested that both DELcore and DELflex lists be generated, but the deck in the input file, {self.library.deck}, is not None")    
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
        self.next(self.filter_classnames)

    @step
    def filter_classnames(self):
        if (
            self.library_name in SYMMETRICAL_MONO_BOC_TRIAMINE_LIBRARY_IDS
            and self.cycle.cycle_name == "A"
        ):
            pass  # Cores are registered as TechDevBBs
        else:
            self.cycle.filter_classnames()
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
        self.next(self.filter_tbbs)

    @step
    def filter_tbbs(self):
        self.cycle.filter_tbbs()
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
        self.next(self.filter_virtual_bbs)

    @step
    def filter_virtual_bbs(self):
        self.cycle.filter_virtual_bbs()
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
    ...
    def filter_duplicate_ids(self):
        ...
        self.next(self.calculate_num_stereoisomers)

    @step
    def calculate_num_stereoisomers(self):
        self.cycle.calculate_num_stereoisomers(
            col_name_of_monosynthon_smiles="Monosynthon SMILES"
        )
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
        self.next(self.filter_stereoisomers)

    @step
    def filter_stereoisomers(self):
        self.cycle.filter_stereoisomers()
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
        self.next(self.check_bb_count)

    @step
    def check_bb_count(self):
        self.cycle.check_bb_count()
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
    ...
    def write_lists(self):
        if self.generate_delcore_and_delflex_lists:
            delcore = deepcopy(self.library)
            delflex = deepcopy(self.library)
            
            delcore_cycles = list(delcore.cycles)
            for cycle in delcore_cycles:
                cycle.filter_cycle_for_deck("DELcore")
            setattr(delcore, "cycles", tuple(delcore_cycles))
            delflex_cycles = list(delflex.cycles)
            for cycle in delflex_cycles:
                cycle.filter_cycle_for_deck("DELflex")
            setattr(delflex, "cycles", tuple(delflex_cycles))

            delcore.write_excel_files(
                input_filepath = self.input_filepath,
                output_filepath = self.output_directory
                + self.library.lib_name.replace("_postsynthesis", "").replace(
                    "_presynthesis", ""
                )
                + "_DELcore_output.xlsx",
                qc_set_filepath = None,
                readidel_exemplar_filepath=None,
                color_list_tabs=False,
                is_presynthesis=True,
            )
            delflex.write_excel_files(
                self.input_filepath,
                self.output_directory
                + self.library.lib_name.replace("_postsynthesis", "").replace(
                    "_presynthesis", ""
                )
                + "_DELflex_output.xlsx",
                qc_set_filepath = None,
                readidel_exemplar_filepath=None,
                color_list_tabs=False,
                is_presynthesis=True,
            ...
            self.library.write_excel_files(
                input_filepath = self.input_filepath,
                output_filepath = self.output_directory
                + self.library.lib_name.replace("_postsynthesis", "").replace(
                    "_presynthesis", ""
