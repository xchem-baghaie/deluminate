...
class Postsynthesis_Enumeration(FlowSpec):
    ...
    def query_compatible_bbs(self):
        self.cycle.query_compatible_bbs(is_postsynthesis=True)
        self.log(
            f"Finished step {inspect.stack()[0][3]} for cycle {self.cycle.cycle_name}. Current split size  = {self.cycle.split_size}",
            "info",
        )
    ...
    def monosynthon_enumeration(self):
        ...
        self.cycle.monosynthon_enumeration(
            linkersmiles = self.library.linker_cycle.default_smiles,
            cycle_names = cycle_names,
            placeholder_reaction_branches = placeholder_reaction_branches,
            placeholder_bb_exemplars = placeholder_bb_exemplars,
            compute_xsmiles = self.compute_xsmiles,
    ...
    def write_lists(self):
        if self.library.synonym is not None:
            readidel_exemplar_filepath = str(self.output_directory + self.library.synonym + "_Instance_Exemplars_withIDs.xlsx",)
        else:
        ...
        self.library.write_excel_files(
            ...
            + "_output.xlsx",
            qc_set_filepath = self.output_directory
            + self.library.lib_name.replace("_postsynthesis", "").replace(
                "_presynthesis", ""
            )
            + "_QC_set.csv",
            readidel_exemplar_filepath=readidel_exemplar_filepath,
